...
Stone[]	terrain = new Stone[20];
  ...
  //  goalSound = new SoundFile(this, "goal.mp3");
    ...
    void vhiicleCollision() {
    	...
    	for (Obstacle : this.getest()) {
    	    Obstacle recPlayer = test.playerBounds();
    	        for ( goal()  ) {
    	            DrawRect goalTest = goal.obstBounds();

    	            if (recPlayer.intersects(goalTest)) {
    	                println("sotppie stopp");    	            }
    	        }
    	}
    ...
    void rover()	{
    	PImage roverPix = loadImage("marsRoverPix.png");
    	
    	int roverOffsetX = mouseX-roverSize/2;
    	int roverOffsetY = mouseY-roverSize/2;
    	
    	roverPix.resize(roverSize,roverSize);
    	image(roverPix,roverOffsetX,roverOffsetY);
